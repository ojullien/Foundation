...
class CSeverity extends \PHPUnit_Framework_TestCase
{
    ...
    public function testClass()
    {
        $this->proceed(
            'TEST EMERG',
            \Foundation\Type\Enum\CSeverity::EMERG,
            [ 'getValue'   => 0,
            '__toString' => 'emergency' ]
        );
        $this->proceed(
            'TEST ALERT',
            \Foundation\Type\Enum\CSeverity::ALERT,
            [ 'getValue'   => 1,
            '__toString' => 'alert' ]
        );
        $this->proceed(
            'TEST CRIT',
            \Foundation\Type\Enum\CSeverity::CRIT,
            [ 'getValue'   => 2,
            '__toString' => 'critical' ]
        );
        $this->proceed(
            'TEST ERR',
            \Foundation\Type\Enum\CSeverity::ERR,
            [ 'getValue'   => 3,
            '__toString' => 'error' ]
        );
        $this->proceed(
            'TEST WARN',
            \Foundation\Type\Enum\CSeverity::WARN,
            [ 'getValue'   => 4,
            '__toString' => 'warning' ]
        );
        $this->proceed(
            'TEST NOTICE',
            \Foundation\Type\Enum\CSeverity::NOTICE,
            [ 'getValue'   => 5,
            '__toString' => 'notice' ]
        );
        $this->proceed(
            'TEST INFO',
            \Foundation\Type\Enum\CSeverity::INFO,
            [ 'getValue'   => 6,
            '__toString' => 'informational' ]
        );
        $this->proceed(
            'TEST DEBUG',
            \Foundation\Type\Enum\CSeverity::DEBUG,
            [ 'getValue'   => 7,
            '__toString' => 'debug' ]
        );
    }
