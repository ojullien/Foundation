...
class CByteTest extends \PHPUnit_Framework_TestCase
{
    ...
    private function proceedClass( $label, $value, array $expected )
    {
        ...
        $type   = new \Foundation\Type\Complex\CByte( $value );
        $this->assertSame( $expected['isShorthanded']['return']
                , $method->invokeArgs( $type, array( $value ) )
                , $label . ' isShorthanded' );
        $this->assertSame( $expected['isvalid']['return'], $type->isValid(), $label . ' isValid' );
        $this->assertSame( $expected['getValue']['return'], $type->getValue(), $label . ' getValue' );
        $this->assertSame( $expected['__toString']['return'], (string)$type, $label . ' __toString' );
        $this->assertSame( $expected['getLength']['return'], $type->getLength(), $label . ' getLength' );
        $this->assertSame( $expected['convertToByte']['return']['numeric'], $type->convertToByte(),
                           $label . ' convertToByte' );

        $this->assertSame( $expected['convertToKByte']['return']['numeric'], $type->convertToKByte(),
                           $label . ' convertToKByte' );
        $this->assertSame( $expected['convertToKByte']['return']['shorthanded'], $type->convertToKByte( TRUE ),
                                                                                                        $label . ' convertToKByte SH' );

        $this->assertSame( $expected['convertToMByte']['return']['numeric'], $type->convertToMByte(),
                           $label . ' convertToMByte' );
        $this->assertSame( $expected['convertToMByte']['return']['shorthanded'], $type->convertToMByte( TRUE ),
                                                                                                        $label . ' convertToMByte SH' );

        $this->assertSame( $expected['convertToGByte']['return']['numeric'], $type->convertToGByte(),
                           $label . ' convertToGByte' );
        $this->assertSame( $expected['convertToGByte']['return']['shorthanded'], $type->convertToGByte( TRUE ),
