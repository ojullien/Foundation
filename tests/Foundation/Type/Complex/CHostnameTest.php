...
class CHostnameTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $type = new \Foundation\Type\Complex\CHostname('192.168.33.1', [ true, false, true ]);
        $this->assertFalse($type->isValid(), 'TEST: not authorized \'192.168.33.1\'');
        unset($type);
        $type = new \Foundation\Type\Complex\CHostname('192.168.33.1', [ true, true, true ]);
        $this->assertTrue($type->isValid(), 'TEST: authorized \'192.168.33.1\'');
        unset($type);
        $type = new \Foundation\Type\Complex\CHostname('localhost', [ false, false, false ]);
        $this->assertFalse($type->isValid(), 'TEST: not authorized \'localhost\'');
        unset($type);
        $type = new \Foundation\Type\Complex\CHostname('localhost', [ false, false, true ]);
