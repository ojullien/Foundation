...
class CDecoderWuTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testDecodeJSONEmpty()
    {
        ...
        $return     = $pDecoderWu->decode( array( ), '' );
        $this->assertNull( $return, 'TEST: decode' );
        $this->assertSame( \Foundation\Weather\Decoder\CDecoder::FWD_ERROR_SYNTAX, $pDecoderWu->getErrorNumber(),
                           'TEST: getErrorNumber' );
    ...
    public function testDecodeJSONBad()
    {
        $pDecoder   = new \Foundation\Weather\Decoder\CDecoder();
        $pDecoderWu = new \Foundation\Weather\Decoder\CDecoderWu( $pDecoder );
        $return     = $pDecoderWu->decode( array( ), '{"response":{"version": "0.1"}' );
        $this->assertNull( $return, 'TEST: decode' );
    ...
    public function testCompactKeyMissing01()
    {
        ...
        $pMethod->invokeArgs( $pDecoderWu, array( &$data ) );
        $this->assertSame( \Foundation\Weather\Decoder\CDecoder::FWD_KEY_MISSING, $pDecoderWu->getErrorNumber(),
                           'TEST: getErrorNumber' );
        $this->assertEquals( 'Key "forecast/simpleforecast/forecastday/period" is missing', $pDecoderWu->getErrorText(),
                             'TEST: getErrorText' );
    ...
    public function testCompactKeyMissing02()
    {
        ...
        $this->assertSame( \Foundation\Weather\Decoder\CDecoder::FWD_KEY_MISSING, $pDecoderWu->getErrorNumber(),
                           'TEST: getErrorNumber' );
        $this->assertEquals( 'Key "forecast/simpleforecast/forecastday" is missing', $pDecoderWu->getErrorText(),
                             'TEST: getErrorText' );
        unset( $pDecoderWu, $pDecoder, $pMethod );
    }
    ...
    public function testCompactKeyMissing03()
    {
        ...
        $this->assertSame( \Foundation\Weather\Decoder\CDecoder::FWD_KEY_MISSING, $pDecoderWu->getErrorNumber(),
                           'TEST: getErrorNumber' );
        $this->assertEquals( 'Key "forecast/txt_forecast/forecastday/period" is missing', $pDecoderWu->getErrorText(),
                             'TEST: getErrorText' );
        unset( $pDecoderWu, $pDecoder, $pMethod );
    }
    ...
    public function testCompactKeyMissing04()
    {
        ...
        $this->assertSame( \Foundation\Weather\Decoder\CDecoder::FWD_KEY_MISSING, $pDecoderWu->getErrorNumber(),
                           'TEST: getErrorNumber' );
        $this->assertEquals( 'Key "forecast/txt_forecast/forecastday" is missing', $pDecoderWu->getErrorText(),
                             'TEST: getErrorText' );
        unset( $pDecoderWu, $pDecoder, $pMethod );
    }
    ...
    public function testCompactKeyMissing05()
    {
        ...
        $this->assertSame( \Foundation\Weather\Decoder\CDecoder::FWD_KEY_MISSING, $pDecoderWu->getErrorNumber(),
                           'TEST: getErrorNumber' );
        $this->assertEquals( 'Key "forecast/txt_forecast/date" is missing', $pDecoderWu->getErrorText(),
                             'TEST: getErrorText' );
        unset( $pDecoderWu, $pDecoder, $pMethod );
    }
    ...
    {
        // Connector
        $aConnectorOptionsConnect = array(
