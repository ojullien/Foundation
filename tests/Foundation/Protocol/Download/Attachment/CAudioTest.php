...
class CAudioTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testFromExtensionException()
    {
        foreach( static::$_aFromExtensionException as $data )
        {
            ...
            try
            {
                $pObject = new \Foundation\Protocol\Download\Attachment\CAudio( static::$_pDownloadManager,
                                                                                $test['value'] );
                unset( $pObject );
    ...
    {
        $method = new \ReflectionMethod( '\Foundation\Protocol\Download\Attachment\CAudio',
