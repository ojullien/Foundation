...
final class CDebugger
{
    ...
    public function render(\Foundation\Debug\Render\RenderInterface $pRender, $iDisplay = 0)
    {
        ...
        $sReturn .= $pRender->renderUsage(
            $this->_pTimeManager->getScriptDuration(),
        ...
        if ($iDisplay & self::QUERY) {
            $sReturn .= $pRender->renderVariable('get', $this->_pGet->compare(isset($_GET) ? $_GET : []));
            $sReturn .= $pRender->renderVariable('post', $this->_pPost->compare(isset($_POST) ? $_POST : []));
        }
        if ($iDisplay & self::COOKIE) {
            $sReturn .= $pRender->renderVariable('cookie', $this->_pCookie->compare(isset($_COOKIE) ? $_COOKIE : []));
        }
        if ($iDisplay & self::SESSION) {
            $sReturn .= $pRender->renderSessionConfiguration();
            $sReturn .= $pRender->renderVariable(
                'session',
                $this->_pSession->compare(isset($_SESSION) ? $_SESSION : [])
            );
        }
...
\Foundation\Debug\CDebugger::getInstance()->setVariableCookie(
    new \Foundation\Debug\Variable\CContainer((isset($_COOKIE)) ? $_COOKIE : [])
);
\Foundation\Debug\CDebugger::getInstance()->setVariableSession(
    new \Foundation\Debug\Variable\CContainer((isset($_SESSION)) ? $_SESSION : [])
);
\Foundation\Debug\CDebugger::getInstance()->setVariableGet(
    new \Foundation\Debug\Variable\CContainer((isset($_GET)) ? $_GET : [])
);
\Foundation\Debug\CDebugger::getInstance()->setVariablePost(
    new \Foundation\Debug\Variable\CContainer((isset($_POST)) ? $_POST : [])
);
