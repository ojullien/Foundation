...
final class CMemorizer
{
     ...
     */
    public function __construct(\Foundation\Debug\Timer\TimerInterface $pTimeManager, \Foundation\Debug\Memory\MemoryInterface $pMemoryManager)
    {
    ...
    public function add($sId, $sClass, array $aParams)
    {
        ...
        // Add to the stack
        $this->_aStack[$sId] = [ 'class'      => $sClass . $sParameter,
            'time'       => [ 'start' => $fTime, 'end'   => 0.0 ],
            'memoryPeak' => [ 'start' => $iMemoryPeak, 'end'   => 0 ],
            'memory'     => [ 'start' => $iMemory, 'end'   => 0 ] ];
    }
    ...
    public function delete($sId)
    {
        ...
        if (array_key_exists($sId, $this->_aStack)) {
            ...
            // Add to the stack
            $this->_aStack[$sId] = [ 'class'      => 'UNKNOWN',
                'time'       => [ 'start' => null, 'end'   => $fTime ],
                'memoryPeak' => [ 'start' => 0, 'end'   => $iMemoryPeak ],
                'memory'     => [ 'start' => 0, 'end'   => $iMemory ] ];
        }
    }
}
